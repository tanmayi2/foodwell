...
import { Recipe, RecipeList, UserRecipeData } from "@/types";
...
export function RecipeProvider({ children }: { children: React.ReactNode }) {
  const [userRecipeData, setUserRecipeData] =
    useState<UserRecipeData>(mockUserRecipeData);
    ...
    setUserRecipeData((prev) => ({
      ...
      custom_lists: prev.custom_lists.map((list) =>
        list.id === listId
          ? {
              ...list,
              recipe_ids: list.recipe_ids.filter((id) => id !== recipeId),
            }
          : list
      ),
    }));
  ...
  const getFavoriteRecipes = (allRecipes: Recipe[]): Recipe[] => {
    return allRecipes.filter((recipe) =>
      userRecipeData.favorites.includes(recipe.id)
    );
  };
